...
const slotDateFormat = (slotDate) => {
  ...
  const cancelAppointment = async (appointmendId) =>{
    try {
      ...
      const {data} = await axios.post(backendUrl + "/api/user/cancel-appointment",{appointmendId},{headers:{token}})
      if (data.success) {
        ...
        <div>
            {appointments.map((item,index)=>(
              <div className='grid grid-cols-[1fr_2fr] gap-4 sm:gap-6 py-2 border-b' key={index}>
                ...
                {!item.canclled && item.payment && !item.isCompleted && <button className='sm:min-w-48 py-2 border rounded text-stone-500 bg-indigo-50'>Paid</button>}
